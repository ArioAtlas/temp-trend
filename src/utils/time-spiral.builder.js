...
export class TimeSpiralBuilder {
    ...
    style(_) {
        return arguments.length
            ? ((this._style = Object.assign(this._style, _)), this)
            : this._style;
    }
    ...
    field(_) {
        return arguments.length
            ? ((this._field = Object.assign(this._field, _)), this)
            : this._field;
    }
    ...
    _init() {
        ...
        this._layerHeight =
            (max - this._innerRadius) / (this._layers + 1) -
            this._charBox.height;
    ...
    _process() {
        ...
        this._barWidth =
            this._spiralLength / this._data.length / (this._skinny ? 2 : 1);
    }

    _calculateBars() {
        const bars = this._data.map((d) => {
            const date = new Date(d[this._field.date]),
    ...
    _initScales() {
        ...
        this._time = d3
            .scaleTime()
            .domain([new Date(this._firstDay), new Date(this._lastDay)])
    ...
    _renderSpiral() {
        ...
        this._spiral = this._g
            ...
            .attr(
                'd',
                this._axisSpiral(this._precision * 2 * this._layers + 1)
            );
    }
    ...
    _axisSpiral(length) {
        return d3
            ...
            .radius(
                (d, i) =>
                    (i * (this._maxRadius - this._innerRadius)) / length +
                    this._innerRadius
            )({ length });
    }
    ...
    _renderAxis() {
        ...
        const data = ticks.map((d) => {
            ...
            if (bar && this._centered) {
                ...
                bar.offset =
                    (offset ? offset.height : this._layerHeight) / 2 + w;
            }
            return bar;
        });
